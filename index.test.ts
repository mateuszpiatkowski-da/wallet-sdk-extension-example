...
test('plugin connection', async () => {
  ...
  const darFiles = await Promise.all(
    (await execa('bash', ['./scripts/latest-dars.sh'], { cwd: __dirname })).stdout
      .split('\n')
      .filter(Boolean)
      .map(async (file) => ({
        file: path.join(__dirname, 'dar', file),
        packageId: (await import(path.join(__dirname, 'src', 'codegen', file.replace('.dar', ''), 'lib', 'index.js')))
          .packageId,
      })),
  );
